...
const S = {
  ...
  externalToolsNote: {
    fa: "ابزارهای خط‌فرمانِ خارجی که ایجنت می‌تونه ازشون استفاده کنه — نصبشون کن تا در دسترسِ ایجنت (از طریق اجرای دستور) قرار بگیرن.",
    en: "External command-line tools the agent can use — install them to make them available to the agent (via running commands).",
    ru: "Внешние инструменты командной строки, которые может использовать агент — установите их, чтобы агент мог их запускать.",
    ja: "エージェントが使える外部コマンドラインツールです — インストールするとエージェントがコマンド実行経由で使えるようになります。",
    zh: "代理可以使用的外部命令行工具 — 安装后代理即可通过运行命令使用它们。",
    tr: "Ajanın kullanabileceği harici komut satırı araçları — ajanın komut çalıştırarak erişebilmesi için kurun.",
  },
  installButton: {
    fa: "نصب",
    en: "Install",
